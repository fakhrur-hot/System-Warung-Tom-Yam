...
/**
 * Verifies the SHA-256 signing utility used for Shopee Affiliate Open API authentication.
 *
 * The expected hex digests below were computed independently of [ShopeeAuthSigner] (via
 * `sha256sum`/`openssl dgst -sha256`, cross-checked against each other) over the exact base
 * string Shopee's documented scheme specifies: `appId + timestamp + payload + secret`. A test
 * that only checked the signer against its own output would pass even if the algorithm itself
 * were wrong — which is exactly what happened before this bugfix (the signer computed an HMAC
 * instead of a plain hash and every test still went green).
 */
class ShopeeAuthSignerTest {
    ...
    fun `sign matches an independently computed SHA-256 digest for a simple base string`() {
        // baseString = "a" + "1" + "b" + "c" = "a1bc"
        // Verified independently: `printf '%s' 'a1bc' | sha256sum` and `openssl dgst -sha256`.
    ...
    fun `sign matches an independently computed SHA-256 digest for a realistic payload`() {
        // baseString = appId + timestamp + payload + secret
        //            = app1231700000000{"query":"test"}secret456
        // Verified independently: `printf '%s' '...' | sha256sum` and `openssl dgst -sha256`.
