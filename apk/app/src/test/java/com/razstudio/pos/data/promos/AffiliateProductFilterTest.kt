...
/**
 * Verifies AffiliateProductFilter validation and rotation logic.
 * Uses Robolectric because LinkGenerator relies on android.net.Uri.
 */
...
class AffiliateProductFilterTest {
    ...
    fun `validate returns products with valid HTTPS links`() {
        ...
        val result = AffiliateProductFilter.validate(products, "cafe-tableview")

        assertEquals(1, result.size)
        assertTrue(result[0].url.contains("sub_id=cafe-tableview"))
    ...
    fun `validate appends sub_id via LinkGenerator`() {
        ...
        val result = AffiliateProductFilter.validate(products, "warung-tomyam-ambient")

        assertTrue(result[0].url.contains("sub_id=warung-tomyam-ambient"))
