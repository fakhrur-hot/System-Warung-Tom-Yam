...
object KitchenSlipDocument {
    ...
    fun generate(
        ...
        // Items
        for (item in items) {
            sb.appendLine("[L]<b>${item.quantity}x ${sanitize(com.razstudio.pos.util.MenuName.display(item.nameSnapshot))}</b>")
    ...
    fun generatePerCategory(
        ...
        val menuSize = com.razstudio.pos.data.local.KitchenFontSize.menu(menuFontSize)
        val noteSize = com.razstudio.pos.data.local.KitchenFontSize.note(menuFontSize)
        ...
        for ((category, categoryItems) in grouped) {
            ...
            for (item in categoryItems) {
                sb.appendLine("[L]<b><font size='$menuSize'>${item.quantity}x ${com.razstudio.pos.util.MenuName.display(item.nameSnapshot)}</font></b>")
                if (!item.note.isNullOrBlank()) {
