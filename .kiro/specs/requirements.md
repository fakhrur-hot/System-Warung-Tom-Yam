...
#### Customer-Facing Website — 5 Language Options
- MUST: When a customer scans a table QR and the ordering page loads, a language selector is presented with five choices: **English (EN, default)**, **Malay (BM)**, **Simplified Mandarin (中文)**, **Tamil (தமிழ்)**, and **Thai (ไทย)**.
...
- MUST: The system maintains **translation dictionaries** keyed off the English source: one set for **static strings** (UI labels, buttons, status messages, category names, report labels) covering BM/中文/தமிழ்/ไทย for the website and BM for operational output, and one for **menu content** (food/beverage terms).
...
- MUST: Research and specify the **direct dictionary-translation** mechanism that backs REQ-9: English is the single authored source; BM/中文/தமிழ்/ไทย (website) and BM (operational) are derived by dictionary lookup.
