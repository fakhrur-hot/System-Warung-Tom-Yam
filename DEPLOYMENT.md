...
## 3. Cloudflare Pages (auto-deploy on push to each café branch)

Each café has its own Cloudflare Pages project, deployed from its own branch (e.g.
`tani-tom-yam`). Cloudflare Pages' native git integration builds and deploys the branch
automatically — no GitHub Actions deploy step is required.
...
| Variable | Value |
|---|---|
| `CAFE_BRANCHES` | comma-separated list of café branches to sync, e.g. `tani-tom-yam,sri-pantai-timur` |
| `CLOUDFLARE_PROJECT_NAME` | *(optional, for local scripts)* your café's Cloudflare Pages project name (→ `<name>.pages.dev`) |
...
### Distributing product updates to café branches

The `.github/workflows/distribute-main.yml` workflow runs on every push to `main`. It
reads `CAFE_BRANCHES` and opens a sync PR from `main` into each listed café branch. If a
clean merge is not possible, it opens an issue instead. Review and merge each PR to bring
product updates into that café's deployed site.
