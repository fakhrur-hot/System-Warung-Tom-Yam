 ...
 *
 * Ported from `apk/app`'s `com.razstudio.pos.data.promos.ShopeeAffiliateApiImpl` (post-bugfix
 * version — endpoint/signature/header were all fixed there; see
 * .kiro/specs/shopee-affiliate-ads-apk/bugfix). Requests are authenticated using [ShopeeAuthSigner]
 * (plain SHA-256 of `appId + timestamp + payload + secret`). Credentials (APP_ID and SECRET) are
 * read from BuildConfig fields injected via `local.properties`. If credentials are missing or
 * blank, all methods return [ApiResult.Error] with code `MISSING_CREDENTIALS` — the module
