...
import com.razstudio.opsapp.ui.viewmodels.AffiliateDebugViewModel

/**
 * Affiliate catalog debug/inspection screen — API Query Tester, Link Generator, Cache Inspector,
 * Sync Trigger.
 *
 * Ported from `apk/app`'s `com.razstudio.pos.ui.screens.AffiliateDebugScreen`. There it is gated
 * behind `BuildConfig.DEBUG`; here it is a permanent screen (Requirement 7) — the audience is
 * RAZStudio support staff, not café operators, so no gate is needed.
 */
...
fun AffiliateDebugScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            Button(onClick = onOpenCatalogEditor, modifier = Modifier.fillMaxWidth()) {
                Text("Open Catalog Editor (partners.json)")
            }
