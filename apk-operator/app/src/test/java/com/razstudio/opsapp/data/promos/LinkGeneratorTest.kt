...
/**
 * Ported from `apk/app`'s `com.razstudio.pos.data.promos.LinkGeneratorTest`. Uses Robolectric
 * because `android.net.Uri` is required for URL manipulation.
 */
...
class LinkGeneratorTest {
    ...
    fun `generate appends sub_id and af_id to a base URL`() {
        ...
        assertTrue(result.contains("sub_id=operator-catalog"))
